		...
		switch ($type) {
			case "JAN":
				...
				self::$menu->setListener(function (Player $player, Item $itemClicked, Item $itemClickedWith, SlotChangeAction $action) {
					if ($action->getSourceItem()->getNamedTag()->hasTag("reward")) {
						self::addSound($player,"note.bd",1);
						MonthlyCrates::getInstance()->getScheduler()->scheduleRepeatingTask(new MonthlyCratesTask($player, $action->getInventory(), $action,), 20);
					}
					if ($action->getSourceItem()->getNamedTag()->hasTag("specialReward")) {
						if(isset(MClISTENER::$isOpening[$player->getName()])) {
							...
							self::$menu->getInventory()->setItem($action->getSlot(), self::randFinalItems("finalitems"));
							$player->getInventory()->addItem(self::$menu->getInventory()->getItem($action->getSlot()));
