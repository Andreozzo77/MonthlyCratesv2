...
//was hella lazy to get every single slot my self thx verge
	 ...
	 */
	public function __construct(Player $player, Inventory $inv, SlotChangeAction $slot)
	{
		$this->player = $player;
		$this->inv = $inv;
		$this->slot = $slot;
	...
	public function onRun(int $currentTick)
	{
		...
		if ($this->seconds === 0) {
			if ($this->player->isOnline()) {
				...
				$this->player->getInventory()->addItem($this->inv->getItem($this->slot->getSlot()));
